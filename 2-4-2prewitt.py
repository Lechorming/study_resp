...
# 加载图像
img = cv2.imread('E:/imgs/room.png', cv2.IMREAD_GRAYSCALE)
...
# 算子边缘检测
s_x = np.array([[-1, -1, -1], [0, 0, 0], [1, 1, 1]])
s_y = np.array([[-1, 0, 1], [-1, 0, 1], [-1, 0, 1]])
edge_x = cv2.filter2D(ori_img, -1, s_x)
edge_y = cv2.filter2D(ori_img, -1, s_y)
...
if a==1:
    _, binary_img_gme = cv2.threshold(grad_mod_euclidean, 13, 127, cv2.THRESH_BINARY)
    _, binary_img_gmm = cv2.threshold(grad_mod_manhattan, 18, 255, cv2.THRESH_BINARY)
elif a==0:
    _, binary_img_gme = cv2.threshold(grad_mod_euclidean, 13, 255, cv2.THRESH_BINARY)
    _, binary_img_gmm = cv2.threshold(grad_mod_manhattan, 17, 255, cv2.THRESH_BINARY)
